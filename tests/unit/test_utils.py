...
def test_eleva_quadrado_negativo(test_input, exc_class, msg):
    with pytest.raises(exc_class) as exc:
        eleva_quadrado(test_input)

    assert exc.type == exc_class
...
def test_requires_role_fail(mocker):
    ...
    #Then
    assert result == ({"message": "Acesso negado. Apenas administradores podem acessar esta rota."}, HTTPStatus.FORBIDDEN)
